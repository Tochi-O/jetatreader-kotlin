...
fun ShowSimpleForm(book: MBook,
    ...
    SimpleForm(defaultValue = if(book.notes!!.isNotEmpty()) {book.notes.toString()}
                else {"No thoughts available."}){ note ->
            notesText.value = note
        Log.d("SHOW ME NOTES", "ShowSimpleForm: ${notesText.value}")
        ...
        RoundedButton(label = "Update") {
            if (bookUpdate){
                FirebaseFirestore.getInstance()
                    ...
                    //Log.d("tas", "ShowSimpleForm: ${task.result.toString()}")
                    }.addOnFailureListener {
                        Log.w("Error", "ShowSimpleForm: document unupdated",it )
                    }
            }
